...
def _planned_providers(providers, atype: str, active: bool) -> list[str]:
    return [p.name for p in providers if p.supports(atype) and (active or not p.active)]
...
def _dry_run_json(
    ...
    for artifact, atype in tasks:
        ...
        enabled = _planned_providers(providers, atype, active)
...
def _record(
    ...
    enrich_run.record_provider(provider_name, fetched.source_url)
    if not json_output:
        print(f"  + {provider_name:<11} {artifact}: {len(fetched.findings)} finding(s)")
...
def run(  # noqa: PLR0913
    ...
    *,
    ...
    if dry_run:
        tasks = expand_targets(target, artifact_type, pivots=pivots, max_pivots=max_pivots)
        if json_output:
            print(_dry_run_json(config, target, artifact_type, pivots, max_pivots, active))
        else:
            print(f"osint-recon enrich --dry-run {target} ({artifact_type})")
            print(f"  pivots: {'enabled' if pivots else 'disabled'} (max {max_pivots})")
            print(f"  active tools: {'enabled' if active else 'disabled (use --active)'}")
            for artifact, atype in tasks:
    ...
    enrich_run.classification = "active" if active else "passive"
    if refresh:
    ...
    with Store() as store:
        for index, (artifact, atype) in enumerate(
            ...
            for provider in providers:
                if not provider.supports(atype) or (provider.active and not active):
                    continue
                try:
                    ...
                    continue
                _record(enrich_run, provider.name, artifact, atype, fetched, json_output)

        if shodan_search:
            shodan = next((p for p in providers if isinstance(p, ShodanProvider)), None)
