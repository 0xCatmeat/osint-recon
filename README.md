...
to local tools that touch the target directly, and those are gated behind a
dry-run and an explicit scope record so you do not poke something you should not.
...
- **Passive by default.** Active tooling that touches the target is opt-in and
  runs behind a `--dry-run` plan and a recorded authorization.
...
| dnsx | `~/OSINT/bin/dnsx` | domain | DNS records via local binary. |
| httpx | `~/OSINT/bin/httpx` | domain | HTTP probing via local binary. |
| tlsx | `~/OSINT/bin/tlsx` | domain, IP | TLS certificate data via local binary. |
...
Most providers are passive lookups, but a few optional local ones touch the
target directly: DNS resolution, HTTP probing, TLS probing. The safe habit is to
run `--dry-run` first, read the list of providers it plans to use, and only turn
on active probing where you actually have permission.

The `scope-gate` command writes down that permission before you reach for active
tooling:
...
It drops a JSON record under the reports directory. To be clear about what it
is: an audit trail, not a switch. It records that you were authorized; it does
not launch any scanners itself.
